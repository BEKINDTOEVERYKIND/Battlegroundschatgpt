...
def validate_generation_metadata(generation: dict, data: Path, root: Path = ROOT) -> None:
...
def main() -> int:
    ...
    if not args.data:
        ...
        invariant = ("engine", "referencePackage", "cardsSha256", "rulesetSha256", "trials", "candidates", "seed")
    ...
    validate_generation_metadata(generation, data)
    ...
    provenance.update({"generation": generation, "model_seed": args.model_seed,
    ...
    if not args.pure_model:
        run(["node", "scripts/refine_positions.mjs", "--input", str(data),
             "--proposals", str(proposals), "--out", str(selections)], "search")
    ...
    run(["node", "scripts/evaluate_positions.mjs", "--input", str(data),
         ...
         "--seed", "982451653", "--out", str(fresh)], "evaluate")
