...
from train_recruit_policy import (
    CombatBridge, PolicyFactory, evaluate_policies, play_episode, seed_plan,
...
def main(argv=None):
    ...
    prereg = {"scope": SCOPE, "full_game_ready": False, "config": asdict(config),
        ...
        "independent_test_promotion": "single frozen learner only: paired lower95 > 0 versus practical, zero learner/control failures and timing violations, live snapshot verified; fixture only",
    ...
    factory = PolicyFactory(args.engine_root, json.loads((ROOT / "data/ruleset.json").read_text()), profile)
    ...
    try:
        with gzip.open(args.out / "training_trajectories.jsonl.gz", "wt", encoding="utf8") as archive:
            for iteration in range(1, args.iterations + 1):
                ...
                updates = [] if failures else fit_policy_gradient(actor, batch, config)
        ...
        result = {"scope": SCOPE, "full_game_ready": False, "selection": selection,
            "completed_update_batches": sum(not row["rejected_batch"] for row in training),
            ...
            "frozen_experimental_vs_bc_reference": paired_summary(reference_differences, np.random.default_rng(86028121)),
