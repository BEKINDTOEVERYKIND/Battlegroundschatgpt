...
class TimedRecruitEngine:
    def __init__(self, engine: RecruitEngine, profile: TimingProfile,
        ...
        self._initialize(engine, profile)

    @classmethod
    def for_fixture(cls, engine: RecruitEngine, profile: TimingProfile,
                    ruleset: Mapping[str, Any]):
        """Explicit restricted curriculum entry; never relax full-game gating.

        The concrete early-recruit adapter aborts on unresolved choice expiry.
        Its short synthetic timer is an input assumption, not client validation.
        """
        from bg_ai.hsbrsim_adapter import EarlyRecruitFixtureEngine, FIXTURE_SCOPE
        from bg_ai.recruiting import ruleset_digest
        report = engine.coverage()
        if (not isinstance(engine, EarlyRecruitFixtureEngine) or
                report.get("scope") != FIXTURE_SCOPE or
                report.get("fixture_only") is not True or
                report.get("full_game_ready") is not False or
                report.get("ruleset_sha256") != ruleset_digest(ruleset)):
            raise RecruitCoverageError("An explicitly scoped matching early-recruit fixture is required")
        result = cls.__new__(cls)
        result._initialize(engine, profile)
        result._fixture_ruleset = dict(ruleset)
        return result

    def _initialize(self, engine: RecruitEngine, profile: TimingProfile):
        self.engine = engine
    ...
    def fork(self):
        """Branch the explicit fixture without refunding spent actions/time."""
        if not hasattr(self, "_fixture_ruleset") or not hasattr(self.engine, "fork"):
            raise RecruitCoverageError("Safe replay branching is only available for the early fixture")
