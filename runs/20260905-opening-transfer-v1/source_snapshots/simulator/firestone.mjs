...
// Keep the original adapter available for exact reproduction of frozen v1 data.
export const LEGACY_ADAPTER_VERSION = 'legacy-v1';
export const CURRENT_ADAPTER_VERSION = 'firestone-combat-v2-tribes';
export const CURRENT_TRIBES = TRIBES.map(t => t === 'MECHANICAL' ? 'MECH' : t);
export function normalizeTribe(tribe) {
  const normalized = tribe === 'MECHANICAL' ? 'MECH' : tribe;
  if (![...CURRENT_TRIBES, 'ALL'].includes(normalized)) throw new Error(`Unknown minion tribe: ${tribe}`);
  return normalized;
}
export function normalizeTribes(tribes) { return [...new Set(tribes.map(normalizeTribe))]; }
export function validateAdapterVersion(version) {
  if (![LEGACY_ADAPTER_VERSION, CURRENT_ADAPTER_VERSION].includes(version)) throw new Error(`Unsupported combat adapter version: ${version}`);
  return version;
}
export function adapterVersionForMetadata(metadata = {}) {
  const version = validateAdapterVersion(metadata.adapterVersion ?? LEGACY_ADAPTER_VERSION);
  const schema = metadata.datasetSchemaVersion ?? metadata.schemaVersion ?? 1;
  if (schema !== (version === CURRENT_ADAPTER_VERSION ? 2 : 1)) throw new Error('Combat adapter and dataset schema version mismatch');
  return version;
}
...
export function makeEntity(card, entityId, bonus = 0, { adapterVersion = LEGACY_ADAPTER_VERSION } = {}) {
  ...
  return {
    ...
    races: adapterVersion === CURRENT_ADAPTER_VERSION ? normalizeTribes(card.races ?? []) : card.races ?? [], golden: !!card.premium,
...
export class FirestoneCombat {
  constructor({ referenceCards, ruleset, cardsHash = null, rulesetHash = null, adapterVersion = LEGACY_ADAPTER_VERSION }) {
    if (!Array.isArray(referenceCards) || !referenceCards.length) throw new Error('Empty reference snapshot');
    this.adapterVersion = validateAdapterVersion(adapterVersion);
    this.datasetSchemaVersion = adapterVersion === CURRENT_ADAPTER_VERSION ? 2 : 1;
    this.validTribeNames = adapterVersion === CURRENT_ADAPTER_VERSION ? CURRENT_TRIBES : TRIBES;
    if (adapterVersion === CURRENT_ADAPTER_VERSION) {
      for (const tribe of CURRENT_TRIBES) if (!Number.isInteger(Race[tribe])) throw new Error(`Missing Firestone tribe enum: ${tribe}`);
      referenceCards = referenceCards.map(c => ({ ...c,
        ...(c.races ? { races: normalizeTribes(c.races) } : {}),
        ...(c.race ? { race: normalizeTribe(c.race) } : {}) }));
    }
  ...
  static fromFiles(cardsPath = 'data/reference_cards.json', rulesetPath = 'data/ruleset.json', options = {}) {
    ...
    return new this({ ...options, referenceCards: JSON.parse(cardsBytes), ruleset,
      cardsHash: sha256(cardsBytes), rulesetHash: sha256(ruleBytes) });
  }

  versionMetadata() { return { adapterVersion: this.adapterVersion, datasetSchemaVersion: this.datasetSchemaVersion }; }

  assertMetadata(metadata) {
    if (adapterVersionForMetadata(metadata) !== this.adapterVersion) throw new Error('Combat adapter differs from dataset generation');
  }

  lobbyTribes(validTribes) {
    const tribes = this.adapterVersion === CURRENT_ADAPTER_VERSION && Array.isArray(validTribes) ? validTribes.map(normalizeTribe) : validTribes;
    if (!Array.isArray(tribes) || tribes.length !== 5 || new Set(tribes).size !== 5 || tribes.some(t => !this.validTribeNames.includes(t))) {
  ...
  context(validTribes) {
    if (this.adapterVersion === CURRENT_ADAPTER_VERSION) validTribes = this.lobbyTribes(validTribes);
  ...
  evaluate({ board, opponent, validTribes, heroId, opponentHeroId = heroId, turn = 8,
    ...
    playerGlobalInfo = {}, opponentGlobalInfo = {}, adapterVersion, datasetSchemaVersion }) {
    if (!Number.isInteger(trials) || trials < 1) throw new Error('trials must be a positive integer');
    // Direct callers choose an engine explicitly; dataset callers carry metadata.
    if (adapterVersion != null || datasetSchemaVersion != null) this.assertMetadata({ adapterVersion, datasetSchemaVersion });
    validTribes = this.lobbyTribes(validTribes);
    if (this.adapterVersion === CURRENT_ADAPTER_VERSION) {
      const canonical = entity => ({ ...entity, races: normalizeTribes(entity.races ?? this.reference.get(entity.cardId)?.races ?? []) });
      board = board.map(canonical);
      opponent = opponent.map(canonical);
    }
  ...
  supportReport() {
    ...
    return { ...this.versionMetadata(), engine: ENGINE_VERSION, referencePackage: REFERENCE_VERSION, cardsSha256: this.cardsHash,
...
export class FirestoneCombatV2 extends FirestoneCombat {
  constructor(options) {
    if (options.adapterVersion != null && options.adapterVersion !== CURRENT_ADAPTER_VERSION) throw new Error('FirestoneCombatV2 requires its current adapter version');
    super({ ...options, adapterVersion: CURRENT_ADAPTER_VERSION });
  }
}
